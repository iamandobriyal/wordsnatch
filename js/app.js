function createGame(event) {
  ...
  const totalletters = document.querySelector('input[name="totalLetters"]').value;
  const ws = new WebSocket(`ws://localhost:8080`);

  ws.onopen = () => {
      console.log("WebSocket connection established");
      // Send a message to the server to create a new game
      ws.send(JSON.stringify({ action: 'createGame', playerName: playerName, timer: timer, total: totalletters}));
  };
